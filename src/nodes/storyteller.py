...
def _build_operational_contract_block(current_chapter: int) -> str:
    ...
    return (
        ...
        f"\n"
        f"[!!!] OUTPUT PROTOCOL — CRITICAL\n"
        f"Your final answer is delivered ONLY by calling the `set_model_response`\n"
        f"tool with two arguments: `prose` (str) and `chapter_meta` (object). DO NOT\n"
        f"emit prose as a plain-text response. DO NOT skip the tool wrapper. The\n"
        f"framework cannot accept your chapter unless it arrives as a structured\n"
        f"set_model_response call. After you finish researching (lore_lookup /\n"
        f"trigger_research) and have your full prose written internally, your\n"
        f"FINAL action is exactly one `set_model_response` call carrying the\n"
        f"complete chapter.\n"
        f"\n"
        f"Schema: prose is a string ({_CHAPTER_MIN_WORDS}-{_CHAPTER_MAX_WORDS} words, NO `# Chapter N`\n"
        f"markdown header — the framework prepends it). chapter_meta is a\n"
        f"ChapterOutput object: {{summary, choices (4 entries, all 4 tiers),\n"
        f"choice_timeline_notes, timeline, canon_elements_used,\n"
        f"power_limitations_shown, stakes_tracking, character_voices_used,\n"
        f"questions (1-2)}}.\n"
        f"\n"
        f"Below this contract are CONSTRAINT BLOCKS. Priority markers:\n"
        ...
        f"PLAYER DIRECTIVE [!!!] is the primary action of THIS chapter.\n"
    )
...
def _build_chapter_output_reminder_block(current_chapter: int) -> str:
    ...
    return (
        f"[!!!] FINAL OUTPUT REMINDER\n"
        f"\n"
        f"Your one and only delivery mechanism is a `set_model_response` tool\n"
        f"call. NOT a plain-text response. NOT a markdown block. Call:\n"
        f"\n"
        f"  set_model_response(\n"
        f"    prose=<the chapter body, {_CHAPTER_MIN_WORDS}-{_CHAPTER_MAX_WORDS} words, NO `# Chapter N` header>,\n"
        f"    chapter_meta=<ChapterOutput with summary, choices (4 tiers), \n"
        f"      choice_timeline_notes, timeline, canon_elements_used,\n"
        f"      power_limitations_shown, stakes_tracking,\n"
        f"      character_voices_used, questions (1-2)>\n"
        f"  )\n"
        f"\n"
        f"If you emit prose as a top-level text response instead of inside the\n"
        f"set_model_response call, the framework rejects the chapter and the\n"
        f"work is lost. The PLAYER DIRECTIVE [!!!] block above is this chapter's\n"
        f"primary action — enact it inside the prose argument. Do NOT defer it\n"
        f"to a cliffhanger.\n"
    )
